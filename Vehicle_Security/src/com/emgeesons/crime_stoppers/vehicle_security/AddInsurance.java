...
public class AddInsurance extends BaseActivity implements TextWatcher {
	...
	private class ins extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			HttpPost httppost = new HttpPost(ins_url);
			@SuppressWarnings("deprecation")
			...
			try {
				...
				mpEntity.addPart("insuranceCompanyName", new StringBody(name));

				mpEntity.addPart("insuranceCompanyNumber", new StringBody(
						number.getText().toString()));

				mpEntity.addPart("insurancePolicyNumber", new StringBody(policy
						.getText().toString()));
				if (expiry.getText().toString().isEmpty()) {
					mpEntity.addPart("insuranceExpiryDate", new StringBody(
							"000-00-00"));
